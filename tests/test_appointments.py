...
def test_crear_appointment_body_no_json(client, auth_token):
    ...
    res = client.post('/api/v1/appointments',
        data="no es un JSON",
...
def test_get_appointment_inexistente(client, auth_token):
    ...
    res = client.get('/api/v1/appointments/999', headers={
...
def test_get_appointments_con_token(client, auth_token):
    ...
    res = client.get('/api/v1/appointments', headers={
        "Authorization": f"Bearer {auth_token}"
    })
...
def test_get_appointment_por_id(client, auth_token, appointment_id):
    ...
    res = client.get(f'/api/v1/appointments/{appointment_id}', headers={
        "Authorization": f"Bearer {auth_token}"
    })
...
def test_actualizar_appointment(client, auth_token, appointment_id):
    ...
    res = client.put(f'/api/v1/appointments/{appointment_id}', json={
...
def test_eliminar_appointment(client, auth_token, appointment_id):
    ...
    res = client.delete(f'/api/v1/appointments/{appointment_id}', headers={
        "Authorization": f"Bearer {auth_token}"
    })
