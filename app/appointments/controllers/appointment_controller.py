...
def get_appointments():
    ...
    appointments = (
        Appointment.query
        .join(Pet)
        .filter(Pet.owner_id == current_user_id)
        .all()
    )
...
def create_appointment():
    ...
    if not title or not date or not type_ or not pet_id:
        return jsonify({
            "error": "title, date, type y pet_id son obligatorios"
        }), 422
...
def update_appointment(appointment_id):
    ...
    if "type" in data:
        appointment.type = data["type"].strip()
